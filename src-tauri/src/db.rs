...
impl Database {
    ...
    pub fn get_conversations_for_export(
        &self,
        app_id: Option<&str>,
        start_date: Option<&str>,
        end_date: Option<&str>,
        keyword: Option<&str>,
    ) -> Result<Vec<ExportConversationRecord>, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        ...
        if let Some(sd) = start_date {
            if !sd.is_empty() {
                conditions.push("c.created_at >= strftime('%s', ?)".to_string());
                params.push(Box::new(sd.to_string()));
            }
        }
        if let Some(ed) = end_date {
            if !ed.is_empty() {
                conditions.push("c.created_at < strftime('%s', ?, '+1 day')".to_string());
                params.push(Box::new(ed.to_string()));
            }
        }
        if let Some(kw) = keyword {
            if !kw.is_empty() {
                conditions.push("(c.name LIKE ? OR c.summary LIKE ? OR c.conversation_id LIKE ? OR c.from_end_user_id LIKE ? OR c.from_end_user_session_id LIKE ?)".to_string());
                let pattern = format!("%{}%", kw);
                params.push(Box::new(pattern.clone()));
                params.push(Box::new(pattern.clone()));
                params.push(Box::new(pattern.clone()));
        ...
        let sql = format!(
            "SELECT c.id, c.conversation_id,
             COALESCE(NULLIF(c.name, ''), c.conversation_id) AS title,
             ...
             COALESCE(c.status_count, '{{}}') AS status_count,
             ...
             ) mc ON mc.app_id = c.app_id AND mc.conversation_id = c.conversation_id
    ...
    pub fn batch_upsert_messages_multi(&self, app_id: &str, msgs: &[(String, DifyMessageItem)]) -> Result<usize, String> {
        if msgs.is_empty() {
            return Ok(0);
        }
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        conn.execute_batch("BEGIN TRANSACTION").map_err(|e| e.to_string())?;
        let mut count = 0usize;
        for (conversation_id, msg) in msgs {
            let id = format!("{}:{}", app_id, msg.id);
            let feedback_rating = msg.feedback.as_ref().map(|f| f.rating.clone())
                .or_else(|| feedback_rating_from_array(&msg.feedbacks));
            let mut retriever_resources = msg.retriever_resources.clone();
